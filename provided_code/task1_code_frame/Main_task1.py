...
numEpochs = 5           # number of epoch run in total
...
if not osp.exists("model.pt"):
    ...
    # Add the produced 'test_results.txt' file to your sciebo team folder
    # after analyzing the final evaluation datasets
        ...
        with torch.no_grad():
            ...
            with open("test_results.txt", "w") as file:
                ...
                for set_name, ds in zip(set_names, test_sets):
                    ...
                    count_neg = 0
                    for img_pos in ds:
                        pred = model(img_pos.to(device)).item()
                        if pred > 0.5:
                            count_pos += 1
                        else:
                            count_neg += 1
